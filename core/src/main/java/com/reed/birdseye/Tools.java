...
public class Tools {
    ...
    void draw(SpriteBatch batch) {
        // draw sword
        if (Tutorial.step >= 7) {
            batch.draw(Assets.toolsMasterAtlas.findRegion("w_shortsword_0"), startOfTopBar + 3 * boxWidth + toolX,
                    Gdx.graphics.getHeight() - toolY, 70, 70);
        }
        // draw pick axe
        batch.draw(Assets.toolsMasterAtlas.findRegion("pick"), startOfTopBar + 0 * boxWidth + toolX,
                Gdx.graphics.getHeight() - toolY, 50, 50);
        if (Tutorial.step >= 5) {
            // draw hatchet
            batch.draw(Assets.toolsMasterAtlas.findRegion("gear_swords"), startOfTopBar + 2 * boxWidth + toolX,
                    Gdx.graphics.getHeight() - toolY, 50, 50);
        }
        if (Tutorial.step >= 2) {
            // draw fishing rod
            batch.draw(Assets.toolsMasterAtlas.findRegion("fishingrod"), startOfTopBar + 1 * boxWidth + toolX,
